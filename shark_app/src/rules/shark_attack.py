...
def update_book(request: Request, id: str, book: UpdateBooks):
    ...
    if len(book) >= 1:
        update_result = get_collection_books(request).update_one(
    ...
    if (
        existing_book := get_collection_books(request).find_one({"_id": ObjectId(id)})
    ) is not None:
...
def list_books_by_author(request: Request, author: str):
    books = list(
        get_collection_books(request).aggregate([{"$match": {"author": author}}])
    )
    return books
